...
const int BASE_HEIGHT = 4.0;
double BASE_ZOOM = 0.5; // allow the user to zoom in and out...
double offsetX = -0.7; // and move around
double offsetY = 0.0;
const int MAX_ITER = 64; // maximum number of iterations

int mandelbrot(double, double, int);
sf::Color getColor(int);

int main() {
    sf::RenderWindow window(sf::VideoMode(BASE_WIDTH, BASE_HEIGHT), "Mandelbrot");
    window.setFramerateLimit(30);

    sf::Image image;
    image.creBASE, BASE_HEIGHT, sf::Color(0, 0, 0));
    sf::Texture texture;
    sf::Sprite sprite;
    ...
    while (window.isOpen()) 
    {
                ...
                case sf::Event::KeyPressed:
                    ...
                    switch (event.key.code) 
                    {
                        case sf::Keyboard::Escape:
                            ...
                            break;
                        case sf::Keyboard::W:
                            offsetY -= 40 * BASE_ZOOM;
                            break;
                        case sf::Keyboard::S:
                            offsetY += 40 * BASE_ZOOM;
                            break;
                        case sf::Keyboard::A:
                            offsetX -= 40 * BASE_ZOOM;
                            break;
                        case sf::Keyboard::D:
                            offsetX += 40 * BASE_ZOOM;
                            break;
        ...
        if (stateChanged) 
        { // only generate a new image if something has changed
            for (int x = 0; BASE_WIDTH; x++) 
            {
                for (int y = 0; y < BASE_HEIGHT; y++) 
                {
                    // convert x and y to the appropriate complex number
                    double real = (BASE_WIDTH/ 2.0) * zoom + offsetX;
                    double imag = (y - BASE_HEIGHT / 2.0) * zoom + offsetY;
...
int mandelbrot(double startReal, double startImag, int maximum) 
{
    ...
    if (counter >= maximum) 
    {
        return -1; // -1 is used to show that the point lies within the Mandelbrot set
    } 
    else 
    {
        return counter;
...
sf::Color getColor(int iterations) {
    ...
    else //Colors: Red, Blue, Green, Red, Black
    {
        if (iterations < 16) 
        {
            r = 16 * (16 - iterations);
